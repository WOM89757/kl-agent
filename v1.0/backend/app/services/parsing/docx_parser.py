...
def iter_block_items(doc):
    """
    你项目里如果已经有 iter_block_items，可直接复用。
    这里只保留调用约定，不展开底层 XML 实现。
    """
...
class DocxParser(BaseParser):
    ...
    ARTICLE_PATTERNS = [
        re.compile(r"^\s*第[一二三四五六七八九十百千万0-9]+条\b.*$"),
    ]
                ...
                if block_type in {"chapter", "appendix"}:
                    ...
                    blocks.append(
                        Block(
                            ...
                            text=text,
                            level=level,
                            path=self._path_from_hierarchy(hierarchy),
                            heading=text,
                            metadata={"kind": block_type},
                        )
                    )
                    ...
                    if current_block is None:
                        ...
                        current_block = Block(
                            type="paragraph",
                            text=text,
                            level=(hierarchy[-1][0] + 1) if hierarchy else 0,
                            path=self._path_from_hierarchy(hierarchy),
                            heading=hierarchy[-1][1] if hierarchy else None,
    ...
    def _classify_paragraph(self, text: str) -> Tuple[str, int]:
        ...
        level 越小表示层级越高
        """
        if self._match_any(text, self.CHAPTER_PATTERNS):
            return "chapter", 1

        if self._match_any(text, self.APPENDIX_PATTERNS):
            return "appendix", 1

        if self._match_any(text, self.ARTICLE_PATTERNS):
            return "article", 2

        if self._match_any(text, self.SECTION_PATTERNS):
            return "section", 3

        if self._match_any(text, self.SUBSECTION_PATTERNS):
            return "subsection", 4

        if self._match_any(text, self.ITEM_PATTERNS):
            return "item", 5
    ...
    def _path_from_hierarchy(self, hierarchy: List[Tuple[int, str]]) -> List[str]:
